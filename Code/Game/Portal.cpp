...
void Portal::Update_MoveCamera()
{
	...
	//// Convert player position from world space to other portal space
	//Mat44 otherPortalMatrixWorldToModel = m_otherPortal->GetWorldToModelTransform();
	//Vec3 playerPosInOtherPortalSpace = otherPortalMatrixWorldToModel.TransformPosition3D(playerPos);

	//// Convert the player position back to world space with reference to self portal, instead of the other portal
	//Mat44 selfMatrixModelToWorld = GetModelToWorldTransform();
	//Vec3 playerPositionInSelfPortalSpace = selfMatrixModelToWorld.TransformPosition3D(playerPosInOtherPortalSpace);

	//// Calculate the proper orientation in the other portal space
	//EulerAngles playerOrientationInOtherPortalSpace = playerOrientation;
	//playerOrientationInOtherPortalSpace += m_orientation;

	////EulerAngles newOrientation = m_game->m_player->m_worldCamera->GetOrientation();
	//Vec3 cameraToOtherPortal = otherPortalPos - playerPositionInSelfPortalSpace;
	//float lengthOfOtherPortalToCamera = playerPosInOtherPortalSpace.GetLength();

	//m_portalCamera->SetPositionAndOrientation(playerPositionInSelfPortalSpace, playerOrientationInOtherPortalSpace);
	...
	//float agreementBetweenCameraFwdAndCameraToOtherPortal = DotProduct3D(cameraToOtherPortal.GetNormalized(), playerOrientationInOtherPortalSpace.GetForwardDir_IFwd_JLeft_KUp());
	//m_portalCamera->ChangeNearPlane(lengthOfOtherPortalToCamera * agreementBetweenCameraFwdAndCameraToOtherPortal);

	//DebugAddMessage(Stringf("Distance from portal: %.2f", lengthOfOtherPortalToCamera), 0.f, Rgba8::CYAN);
	//DebugAddMessage(Stringf("Agreement between fwdDir and portal: %.2f", agreementBetweenCameraFwdAndCameraToOtherPortal), 0.f, Rgba8::YELLOW);
	//DebugAddMessage(Stringf("Player in other portal space: %.2f, %.2f, %.2f", playerPosInOtherPortalSpace.x, playerPosInOtherPortalSpace.y, playerPosInOtherPortalSpace.z), 0.f, Rgba8::MAGENTA);
	//DebugAddMessage(Stringf("Player in self portal space: %.2f, %.2f, %.2f", playerPositionInSelfPortalSpace.x, playerPositionInSelfPortalSpace.y, playerPositionInSelfPortalSpace.z), 0.f, Rgba8::RED);
	...
	EulerAngles playerOrientationInOtherPortalSpace = playerOrientation;
	playerOrientationInOtherPortalSpace -= m_orientation;
	playerOrientationInOtherPortalSpace += m_otherPortal->m_orientation;
	//Mat44 playerOrientationMatrix = playerOrientation.GetAsMatrix_IFwd_JLeft_KUp();
	//Mat44 inverseOfOtherOrientationMatrix = m_otherPortal->m_orientation.GetAsMatrix_IFwd_JLeft_KUp();
	//inverseOfOtherOrientationMatrix.Transpose();
	//Mat44 selfOrientationMatrix = m_orientation.GetAsMatrix_IFwd_JLeft_KUp();
	//playerOrientationMatrix.Append(selfOrientationMatrix); // Converts it from World space to self portal space
	//playerOrientationMatrix.Append(inverseOfOtherOrientationMatrix); // Converts from self portal space to world space with reference to the other portal space
	//playerOrientationMatrix.Orthonormalize_XFwd_YLeft_ZUp();
	//EulerAngles newCameraOrientation = EulerAngles(playerOrientationMatrix);
	...
	//DebugAddMessage(Stringf("Distance from portal: %.2f", lengthOfCameraToPortal), 0.f, Rgba8::CYAN);
	//DebugAddMessage(Stringf("Camera near plane: %.2f", m_portalCamera->GetPerspectiveNearPlane()), 0.f, Rgba8::GREEN);
	//DebugAddMessage(Stringf("Agreement between fwdDir and portal: %.2f", agreementBetweenCameraFwdAndCameraToPortal), 0.f, Rgba8::YELLOW);
	//DebugAddMessage(Stringf("Player Orientation: %.2f, %.2f, %.2f", playerOrientation.m_yawDegrees, playerOrientation.m_pitchDegrees, playerOrientation.m_rollDegrees), 0.f, Rgba8::MAGENTA);
	//DebugAddMessage(Stringf("New Camera Orientation: %.2f, %.2f, %.2f", playerOrientationInOtherPortalSpace.m_yawDegrees, playerOrientationInOtherPortalSpace.m_pitchDegrees, playerOrientationInOtherPortalSpace.m_rollDegrees), 0.f, Rgba8::RED);
}
